    ...
    return (
        <Box >
            <Tabs
                ...
                sx={{border: '1px solid', borderColor: 'grey.200', borderRadius: '25px', padding: '0 20px', width: '100%', minHeight: '38px'}}
            >
                {tempArray.map((item) => (
                        <Tab key={item} disableFocusRipple disableTouchRipple
                             label={(item%2) === 0 ? <Typography fontSize={'14px'} fontWeight={'600'} noWrap>{inputs[item/2].label}</Typography> : ''}
                             icon={((item%2) !== 0 && item !== maxLength) ? <Divider orientation={"vertical"}/> : ''}
                             disabled={(item%2) !== 0 && item !== maxLength}
                             sx={{minWidth: 0, minHeight: '38px', display: (item !== maxLength ? 'flex' : 'none'),
