...
export default function DataList( props : selectDialogProps) {
    const {data, search, handelItemClick, noDescription} = props;
            ...
            <Grid px={2}>
                {data ? data.filter(item => item.title.toLowerCase().includes(search)).map(item => (
                        <Grid container key={item.id}>
                            <SelectDialogListItem dataItem={item} selectedValue={''} handleListItemClick={handelItemClick} noDescription={noDescription} />
