...
const TrainCardTwo = () => {
    return (
        <>
                ...
                <Grid item container xs={12} sx={{ maxWidth: { md: '1200px' }, bgcolor: 'common.white' }} justifyContent={"center"} >
                    <Grid container xs={12} item flexDirection={"row"} justifyContent={"space-between"} sx={{ overflowX: "auto" }} flexShrink={0}>
                        ...
                        {trainCardDataTwo.map(item => (
                            <Grid item md={item.width} key={item.id} borderRadius={"0.5rem"} flex-direction={"column"} overflow={"hidden"} flexWrap={"wrap"} sx={{ border: 1, borderRadius: '10px 10px 10px 10px', borderColor: 'divider', display: { xs: "none", md: "flex" } }} height={550}>
                                <TrainItem item={item} />
                            </Grid>)
                        )}
                    </Grid>
            ...
            {/* mobile */}
            <Grid sx={{ overflowX: "scroll", display: { xs: "flex", md: "none" }, marginLeft: "1rem", width: { xs: 450, sm: 730 } }} mx={"1rem"} gap={"1rem"}>
