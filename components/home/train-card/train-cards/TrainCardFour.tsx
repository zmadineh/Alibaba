            ...
            <Grid item container xs={12} sx={{ maxWidth: { md: '1200px' } }} justifyContent={"center"} p={2}>
                ...
                <Grid container item flexDirection={"row"} justifyContent={"space-between"} sx={{ overflowX: "auto" }} flexShrink={0} >
                    {trainCardDataFive.map(item => (
                        <Grid item key={item.id} md={item.width} borderRadius={"0.5rem"} flex-direction={"column"} overflow={"hidden"} flexWrap={"wrap"} sx={{ border: 1, borderRadius: '10px 10px 10px 10px', borderColor: 'divider', bgcolor: 'common.white',display: { xs: "none", md: "flex" } }} flexShrink={0} height={450} >
                            <TrainItem item={item} />
                        </Grid>
                    ))}
                </Grid>
